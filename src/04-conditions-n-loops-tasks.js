...
function isCreditCardNumber(ccn) {
  ...
  let evenSum = 0;
  let oddSum = 0;
  const changeArr = newArr.map((item, index) => (index % 2 === 0 ? +item : item * 2));
  const oddArr = changeArr.filter((item, index) => index % 2 !== 0);
  for (let i = 0; i < changeArr.length; i += 1) {
    if (i % 2 === 0) {
      evenSum += changeArr[i];
    }
  }
  oddSum = oddArr.map((item) => (item > 9 ? item - 9 : item)).reduce((acc, item) => acc + item, 0);
...
function getDigitalRoot(num) {
  ...
  let total = arrNum.reduce((acc, item) => acc + item, 0);
  if (total > 9) {
    total = getDigitalRoot(total);
  }
  return total;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 1; i < pathes.length; i += 1) {
    let index = 0;
    while (pathes[0].at(index) === pathes[i].at(index)) {
      index += 1;
    }
    start = pathes[0].slice(0, index);
  }
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < m1.length; i += 1) {
    ...
    for (let j = 0; j < m2[0].length; j += 1) {
      let a = 0;
      for (let x = 0; x < m2.length; x += 1) a += m1[i][x] * m2[x][j];
      arr[i].push(a);
    }
  }
  return arr;
